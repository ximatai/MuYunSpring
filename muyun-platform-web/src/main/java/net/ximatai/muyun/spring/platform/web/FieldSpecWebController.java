    ...
    public ModuleUiDefinition moduleUiDefinition() {
        return ModuleUiDefinition.builder(FieldSpecService.MODULE_ALIAS)
                .page(PageTemplates.flatManagement(page -> page
                        .explorer(explorer -> explorer.title("字段规格列表")
                                .searchPlaceholder("搜索字段规格名称或 alias")
                                .emptyDescription("暂无字段规格").recordLabel("字段规格")
                                .fallbackTitle("未命名字段规格").titleField("title").secondaryField("alias")
                                .mutedWhenDisabled())
                        .detail(detail -> detail.emptyDescription("请选择字段规格，或新建字段规格")
                                .createTitle("新建字段规格").editor(form -> form.title("字段规格")
