...
业务日志留存是运行期平台治理配置。登录、动作、请求异常和页面访问分别保存是否自动清理及保留天数，默认均关闭自动清理并保留 180 天。只有平台范围的系统管理员可以查看、修改策略或立即清理；租户和机构范围的日志读取授权不包含物理清理权限。管理端“日志留存”页面的策略修改即时持久化，定时任务每轮读取最新策略，无需修改部署文件或重启应用。策略更新携带版本并使用乐观锁，过期页面不能覆盖其他管理员刚保存的配置；有未保存修改时，页面不允许立即清理，避免确认内容与服务端实际保留期不一致。
