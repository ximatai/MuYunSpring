...
业务日志留存是运行期平台治理配置。登录、动作、请求异常和页面访问分别保存是否自动清理及保留天数，默认均关闭自动清理并保留 180 天。只有平台范围的系统管理员可以查看、修改策略或立即清理；租户和机构范围的日志读取授权不包含物理清理权限。留存治理作为内部能力嵌入登录、操作和异常日志查询页，不单独发布菜单；操作日志页分别管理动作与页面访问两类策略。策略修改即时持久化，定时任务每轮读取最新策略，无需修改部署文件或重启应用。策略更新携带版本并使用乐观锁，过期页面不能覆盖其他管理员刚保存的配置；有未保存修改时，页面不允许立即清理，避免确认内容与服务端实际保留期不一致。
...
部署配置只控制调度器和数据库负载边界，不再承载业务保留期。`scheduled-enabled` 默认开启；由于四类业务策略默认都关闭自动清理，首次启动不会删除日志。运维可以用该开关紧急暂停自动调度，但不影响平台管理员使用受权的立即清理入口。扫描间隔不得短于一分钟，初始延迟不得为负，非法配置会在启动期失败。保留期为 1 至 36,500 天；单批最多 10,000 条、每类策略每轮最多 1,000 批，默认每类每轮上限为 20,000 条。清理结果写入程序日志，不反向写入正在被清理的业务日志表。
...
- 当前业务日志兼容性字段和索引仍由启动期 schema ensure 增量创建；大体量存量表上线前需要评估锁表时间并通过受控运维窗口执行，后续统一版本化数据库迁移时再接入在线索引策略。
