...
公共事实与 `BusinessLogPublisher`、`BusinessLogStore` 位于 `muyun-ability`。采集方只依赖该中立边界；PostgreSQL 追加写入、游标读取和 schema 初始化位于 `muyun-platform`，由 Starter 在受控启动生命周期装配。存储支持单条、批量、重复事件忽略，以及按时间、事件类型、租户、操作者、操作者机构、模块、动作、动作结果、业务记录 ID、变更来源、错误码、登录结果和 HTTP 状态的游标读取。`operator_organization_id` 是兼容性追加列；历史事件不回填，因此机构范围不会显示其归属无法确认的历史记录。
...
`RuntimeActionBusinessLogEventListener` 只观察 `ACTION_EXECUTED` 与 `ACTION_FAILED`，保留原运行事件的 event ID，并转换为动作成功或失败事实。动作事实同时保留运行事件已经确认的实体别名、单条业务记录 ID 和变更来源，用于从管理日志回溯业务对象并区分业务写入、动作执行、系统处理和平台回写；这些属性没有可靠来源时保持为空。它不改变既有 `RuntimeAuditEventListener` 或运行事件多播语义；发布失败只输出固定诊断。动作耗时和影响数量在没有真实来源时为 `null`，不会用 `0` 伪造统计值。

静态标准 `CrudWeb` 的 insert、update、delete 由 `ActionEndpointInterceptor` 在控制器正常完成或抛出异常后记录一次动作事实；能从动作上下文唯一确认记录 ID 时一并保留，变更来源记为业务写入。动态动作仍只消费运行事件，避免双计数。静态记录的成功表示控制器事务范围已正常返回，失败表示控制器异常，均不从 HTTP 状态推断提交结果。
...
“日志管理”中的“程序日志”是一个系统级静态模块，不参与租户选择。它读取当前 JVM 实例的 `muyun.platform.runtime-log.directory` 目录下可读的 `.log` 与 Boot 滚动产生的 `.log.gz` 归档：所有文件可列出和下载，流式查看只选择最新的未压缩活跃日志。归档保留 Spring Boot 默认的 gzip 压缩策略，按大小或日期滚动，重启不会单独生成归档。目录未配置时接口明确报配置错误，不会回退到工作目录；符号链接、目录外路径和删除/清空操作均不开放。每个流持有一个禁止跟随符号链接的打开文件，快照和游标来自同一次读取，后续追加保留 UTF-8 残片；读取目录必须提供稳定的文件身份。活跃文件删除或同路径替换时按文件身份通知客户端重新连接，截断则下发重置事件。每个实例最多同时保留四个流，单个流最长五分钟。
...
业务日志留存是运行期平台治理配置。登录、动作、请求异常和页面访问分别保存是否自动清理及保留天数，默认均关闭自动清理并保留 180 天。只有平台范围的系统管理员可以查看、修改策略或立即清理；租户和机构范围的日志读取授权不包含物理清理权限。管理端“日志留存”页面的策略修改即时持久化，定时任务每轮读取最新策略，无需修改部署文件或重启应用。

平台按事件发生时间计算排他截止点，仅删除 `occurredAt` 严格早于截止点的事实；等于截止点的事件继续保留。立即清理使用该日志类型当前配置的保留天数，即使其自动清理开关关闭也可以人工执行。每次执行受单批数量和每类策略最大批次数双重限制，达到上限会明确报告未完成并等待下次调度或人工继续，不用一次大事务清空历史数据。PostgreSQL 适配器使用数据库 advisory lock 保证集群内同一时刻只有一个清理执行者。

清理能力拆分为存储中立的 `BusinessLogRetentionStore`、`BusinessLogRetentionPolicyStore`、平台治理服务和 Starter 调度适配。自定义日志存储只有同时显式实现物理清理与策略持久化能力后才会装配完整治理服务；普通 `BusinessLogStore` 不会被假定支持物理删除。日志清理仍是日志专题能力，不包装成通用任务中心。
  ...
  platform:
    business-log:
      retention:
        ...
        batch-size: 1000
        maximum-batches-per-policy: 20
        scan-delay: 24h
        initial-delay: 1m
```

部署配置只控制调度器和数据库负载边界，不再承载业务保留期。`scheduled-enabled` 默认开启；由于四类业务策略默认都关闭自动清理，首次启动不会删除日志。运维可以用该开关紧急暂停自动调度，但不影响平台管理员使用受权的立即清理入口。保留期为 1 至 36,500 天；单批最多 10,000 条、每类策略每轮最多 1,000 批，默认每类每轮上限为 20,000 条。清理结果写入程序日志，不反向写入正在被清理的业务日志表。

## 明确限制

- 不实现消息队列、持久缓冲、数据库故障期间可靠接收、自动重试或多存储分流。
- 不实现外置检索、通用查询 DSL 或跨日志类型的授权读取 API。
- 不实现表分区或长期时间桶聚合；数据规模证明需要时再引入分区生命周期治理。
- 不提供业务日志批量导出；导出必须复用读取范围、脱敏投影和有界异步任务，不能把内部诊断或原始敏感字段直接下载。
