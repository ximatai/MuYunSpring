...
配置治理通过 `platform.low_code_governance` 静态模块 API 开放，不声明独立管理菜单；前端从动态模块详情的“低代码”动作进入模块内治理工作区。当前 Web 层只承接已有治理门面，覆盖配置包健康检查、归档、指针切换、当前/历史版本导出、导入 dry-run、导入草稿、草稿归档，以及无状态模板派生和实例化；具体 URL 见 `WEB_API.md`。
