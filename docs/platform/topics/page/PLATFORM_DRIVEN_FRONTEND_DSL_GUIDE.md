...
主表保存公式由静态服务的 `MainRecordFormulaAbility.mainRecordFormulaRules()` 声明；表单 `.businessRules(rules)` 可复用同一列表生成即时计算。动态模块在“业务规则”治理中应用规则后，运行态按表单声明字段自动投射可执行部分，页面无需再次填写公式。

保存公式中声明的引用字段由服务端按引用 ID 受控读取，当前不会自动转为浏览器的 `selectionProjections`。包含此类输入的保存规则及其依赖计算仍由服务端执行，治理试算沿用同一读取边界；页面交互公式的显式投影契约保持独立。
...
租户身份使用登录租户并隐藏租户列表；由登录身份固定的租户范围不在下游业务导航副标题中重复展示。系统身份展示可访问租户及租户范围提示，只有一个候选时自动选中并保留列表。租户选择不改变登录身份，也不改变其他已打开页面的作用域。系统资源和明确的跨范围治理页面保持各自声明的作用域，不能把未选择租户当成“全部租户”。
