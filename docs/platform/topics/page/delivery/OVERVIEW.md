...
常驻字段查询、递归高级编排及动静查询执行收敛的目标与阶段规划见 [标准列表查询治理](../QUERY_GOVERNANCE.md)。递归请求契约和首轮标准列表交互已交付，以下描述当前查询契约及仍保留的兼容入口。
...
| `quickSearch`         | 在已发布 LIST 配置的可见主关系非虚拟文本字段范围内执行不区分大小写的字面包含搜索，转义 `%`、`_` 和反斜杠；列表与导出保持一致。                        |
