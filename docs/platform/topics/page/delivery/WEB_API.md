...
标准页面宿主在业务导航左侧复用租户列表，不要求业务 UI 声明租户导航。职员页面的机构树和考试页面的业务数据使用相同的租户执行机制；治理中的业务预览仅保留发布状态与重新加载等预览职责。查询、主子表保存、引用和动作继续使用原有接口；切换租户仅替换当前页面的业务会话，旧响应不能进入新租户页面。页面布局与租户候选列表保持挂载，搜索、折叠和滚动状态不因租户切换丢失；新范围的数据在对应业务区域内加载，不重新进入整页启动状态。编辑或执行动作期间禁止切换。租户身份自动使用登录租户并隐藏选择入口；系统身份保留租户列表，唯一候选自动选中。未选租户或没有可访问租户时，不发起要求租户的业务请求。
