...
bootstrap 返回模块入口、客户端类型、主实体别名、权限裁剪后的 resolved 页面配置，以及 `/{moduleAlias}/openapi` 文档入口。它面向静态和动态模块共用的标准运行器；模块运行态 descriptor 由 `/platform.module/{moduleAlias}/context` 提供。resolved 页面字段包含 `fieldForm`，前端可据此识别物理字段和虚拟展示字段。

页面配置本身由配置专题维护：菜单方案、菜单树、字段 UI 类型配置、UI 配置集、UI 配置、UI 字段配置、查询模板和查询项的配置 URL 见 `configuration/WEB_API.md`。页面交付只消费已发布配置和当前用户可见菜单。

### 不经菜单的引用浏览

`GET /platform.module/{moduleAlias}/view-context` 以目标模块 `VIEW` 权限读取现有运行态描述符；它不依赖要求 `MENU` 的 `context`，也不使用候选选择接口代替详情读取。目标记录仍通过 `GET /{moduleAlias}/view/{id}` 标准入口读取，执行目标浏览权限、租户和记录数据范围校验，并返回标准读投影。静态与动态目标的记录不存在或不可见时均返回 404。取得来源读投影或专用候选查询资格不授予目标浏览权。现有平台 `REFERENCE` 动作的权限键继承 `VIEW`；本入口保持该既有授权模型，不引入新的独立引用权限。

`view-context` 返回的动作声明继续使用目标模块的授权结果。取得浏览上下文不授予修改权限；引用打开的标准单据通过原有更新、删除及动作入口执行操作，每个入口独立校验动作权限、记录范围和版本，不要求先取得菜单上下文。动态写入超出记录数据范围返回 403 权限拒绝，版本冲突返回 409。

引用浏览请求保留认证与业务租户，不携带来源页面的菜单、导航选择或页面范围。前端只读展示消费返回的标题，既不通过候选页补查标题，也不为目标字典标题调用要求菜单权限的选项入口。
...
| 方法  | URL                                                                      | 功能 |
| ----- | ------------------------------------------------------------------------ | ---- |
| `GET` | `/platform.module/{moduleAlias}/page-query-summary-catalog`             | 返回编排器可选择的合法 `SUM` 字段、受控分组字段和已注册 contributor，格式为 `{moduleAlias,fields:[{fieldName,title}],groupFields:[{fieldName,title,kind:"OPTION"|"REFERENCE"}],contributors:[{contributorKey,title}]}`。 |
...
| `POST` | `/{moduleAlias}/insert`      | 新增记录；请求体直接为业务记录。                       |
| `POST` | `/{moduleAlias}/update/{id}` | 更新记录；请求体直接为业务记录，`version` 参与乐观锁。 |
...
模块 context 的 `tenantRequired` 表示普通业务执行需要明确租户，普通菜单页面和治理中的业务预览消费同一契约。动态记录模块需要租户；静态模块由交付扫描器根据 CRUD 服务、`SystemScope` 和 `GlobalScopedAbility` 推导，手工构建的静态定义可通过 `tenantRequired` 声明。用户、角色等确需系统工作区的治理模块通过 `@StaticModuleTenantScopePolicy(requireActiveTenant = false)` 显式保留跨范围入口；该声明不切换环境租户，也不免除原有权限和归属校验。
...
标准页面宿主在业务导航左侧复用租户列表，不要求业务 UI 声明租户导航。职员页面的机构树和考试页面的业务数据使用相同的租户执行机制；治理中的业务预览仅保留发布状态与重新加载等预览职责。查询、主子表保存、引用和动作继续使用原有接口；切换租户仅替换当前页面的业务会话，旧响应不能进入新租户页面。页面布局与租户候选列表保持挂载，搜索、折叠和滚动状态不因租户切换丢失；新范围的数据在对应业务区域内加载，不重新进入整页启动状态。编辑或执行动作期间禁止切换。租户身份自动使用登录租户并隐藏选择入口；该固定作用域不在下游业务导航副标题中重复展示，其他上游业务范围提示不受影响。系统身份保留租户列表及租户范围提示，唯一候选自动选中。未选租户或没有可访问租户时，不发起要求租户的业务请求。

模块别名、菜单入口或记录详情目标改变时，宿主重建页面控制会话，不能沿用原模块的业务上下文。同一模块重新加载已发布页面时保留已选租户，并以新的业务会话读取最新描述符。切换或重载失败时，租户选择区保持可操作，业务区域显示错误并支持重试；已退出会话的数据不能重新显示。

要求租户的模块业务动作在服务端统一校验有效租户，系统用户省略租户头也不能退回无约束业务查询。模块发现与页面描述符读取不要求先选择业务租户。
