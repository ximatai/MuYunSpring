...
`GET /platform.module/{moduleAlias}/view-context` 以目标模块 `VIEW` 权限读取现有运行态描述符；它不依赖要求 `MENU` 的 `context`，也不使用候选选择接口代替详情读取。目标记录仍通过 `GET /{moduleAlias}/view/{id}` 标准入口读取，执行目标浏览权限、租户和记录数据范围校验，并返回标准读投影。取得来源读投影或专用候选查询资格不授予目标浏览权。现有平台 `REFERENCE` 动作的权限键继承 `VIEW`；本入口保持该既有授权模型，不引入新的独立引用权限。
