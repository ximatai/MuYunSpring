...
平台启动任务在初始数据与声明同步后，恢复已启用且具有主元数据的动态模块运行态，再安装页面执行计划。恢复只编译已有配置，不执行建表或迁移；尚未配置主元数据的模块继续保留为待配置状态。单个模块配置异常时记录警告并跳过，不阻断其他模块恢复及平台治理入口。
...
引用定义声明的非业务字段投影属于只读输出，随表单回传时由 Web 适配层忽略；已声明业务字段仍执行正常写入校验，未声明的未知字段仍被拒绝。动态新增、编辑和删除复用标准变更结果，变更中的模块名使用当前请求解析出的真实模块 alias。
...
| `POST` | `/{moduleAlias}/navigator/reference/translate` | 按 `ids` 恢复已有选择，最多 100 个不同 ID；使用目标模块 `REFERENCE` 动作的数据权限与当前租户范围，返回可见记录。 |

基础 runtime 只要求引用解析闭环。引用生成和草稿确认归属记录联动专题，不在本文列接口。

引用解析的 `formValues` 保留显式空值，供依赖字段过滤区分未填写与具体值；空值本身不构成请求错误。

候选查询的 `fuzzy` 按名称包含匹配处理，用户输入的 `%`、`_` 作为普通字符；键值精确匹配和已选值翻译保持独立语义。静态与动态引用共用此搜索口径。
