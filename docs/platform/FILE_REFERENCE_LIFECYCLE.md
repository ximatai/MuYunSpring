...
业务应用迁移到该标准入口时，应为每个可上传字段注册 `FileReferenceUploadPolicy`：`supportsField` 声明字段覆盖范围，`supports` 和 `authorize` 基于草稿、文件事实和 `CREATE`/`APPEND`/`REPLACE` 意图执行目录、项目、状态及动作授权。应用原有同路径 `file-transfer/upload-ticket` Controller 在 Policy 已注册并通过契约测试后应删除，避免与平台标准映射产生冲突或使实际请求绕开 Policy。
