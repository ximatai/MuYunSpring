# 平台文档

本文档区用于梳理 MuYunSpring 已形成的平台业务能力，帮助内部建设者快速定位能力边界和交接入口。

这里按业务专题组织内容。专题文档只承担线索功能，不复盘推进过程，也不展开 Java 底层设计。

当前目录是平台内部建设和交接的主入口。临时专项记录和已收口清单不长期保留；如果后续发现新的平台缺口，应优先判断它属于哪个业务专题，只有跨专题且尚未形成稳定边界时才临时记录为技术债或专项治理项。

跨专题边界优先放在平台根文档中沉淀。例如 [审计与专题流水边界](AUDIT_AND_PROCESS_LOG_BOUNDARY.md) 用于判断平台运行审计和业务专题流水的职责分工，[外部写入接入边界](EXTERNAL_WRITE_BOUNDARY.md) 用于约束第三方系统、外围平台和后台集成写入平台的方式，[后台任务与异步批次边界](BACKGROUND_JOB_AND_BATCH_BOUNDARY.md) 用于约束长耗时执行、重试、补偿和进度留痕。

## 阅读方式

每个专题优先包含两类文档：

- `OVERVIEW.md`：说明专题解决什么业务问题、包含哪些主要能力、核心链路和边界。
- `WEB_API.md`：按 URL 维度简要说明 Web 接口用途、关键参数和注意点。

如果专题体量较大，会在专题下继续拆二级目录。二级目录仍遵循相同写法，避免把一篇文档写成过重的设计说明。

## 专题地图

| 专题                                 | 内容范围                                                                                                                                               |
| ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `configuration`                      | 应用、模块、元数据、字段、字典、菜单基础配置，以及配置刷新到运行态。                                                                                   |
| `runtime`                            | 动态记录运行态、模块描述、CRUD、查询、引用、动作基础和动态 OpenAPI。                                                                                   |
| `formula`                            | 表单交互公式、动态记录公式规则、引用投影路径、即时试算与服务端执行边界。                                                                                 |
| `identity-permission`                | 租户、组织、用户、角色、登录、动作授权、数据权限、菜单剪枝和当前用户上下文；角色授权目标见 `topics/identity-permission/ROLE_AUTHORIZATION_DESIGN.md`。 |
| `workflow-task`                      | 工作流定义、发布、实例、审批、任务、分支汇聚、待办、历史、委托和插件挂点。                                                                             |
| `workflow-task/workbench`            | 待办、已办、知会、发起跟踪、委托视角和工作台统计。                                                                                                     |
| `business-automation`                | 平台业务自动化能力的总入口。                                                                                                                           |
| `business-automation/code-rule`      | 编码规则、预览、取号、台账、回收池和动态记录赋码。                                                                                                     |
| `business-automation/data-exchange`  | 导入模板、导入执行、错误文件、导出、子表导出和引用 title 往返。                                                                                        |
| `business-automation/record-linkage` | 生单、目标草稿、来源关系、回写规则、贡献台账和失败诊断。                                                                                               |
| `page`                               | 页面交付与页面交互能力的总入口。                                                                                                                       |
| `page/delivery`                      | 菜单入口、页面 bootstrap、列表查询、汇总、表单保存、附件、查重和引用候选。                                                                             |
| `page/interaction`                   | 关联视图、局部编辑、动态弹窗、动作区块、模块任务和任务检查。                                                                                           |
| `governance`                         | 配置包、健康检查、版本归档、指针切换、迁移 dry-run、导入草稿和模板复用。                                                                               |

## 跨专题边界

模块引用从元数据、页面描述符到选择与只读组件的关系见 [模块引用与前端组件专项治理](topics/page/REFERENCE_COMPONENT_GOVERNANCE.md)，包含平台封装目标、字典独立边界和实施阶段。

身份组织对象的统一选择交互、候选范围及阶段规划见 [身份组织对象选择专项治理](OBJECT_SELECTION_GOVERNANCE.md)，覆盖用户、职员、机构、部门与租户选择，以及日志检索的历史身份边界。

列表查询增强的专项规划见 [标准列表查询治理](topics/page/QUERY_GOVERNANCE.md)，覆盖常驻字段查询、递归高级编排、动静统一查询契约和分阶段验收。

| 文档                                                                        | 内容范围                                                                                                             |
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| [审计与专题流水边界](AUDIT_AND_PROCESS_LOG_BOUNDARY.md)                     | 说明平台运行审计、工作流事件、导入导出结果、配置治理事实、记录联动台账、外部写入和后台任务流水的取舍口径。           |
| [日志系统专项治理](LOGGING_GOVERNANCE.md)                                   | 记录已交付的登录审计、访问与动作统计、请求错误和程序日志核心后端能力，以及存储适配边界和明确限制。 |
| [软删除、回收站与删除日志治理](SOFT_DELETION_AND_RECYCLE_BIN_GOVERNANCE.md) | 说明软删除、业务回收站、递归级联、恢复、唯一键冲突提示与删除运维审计的统一边界。                                     |
| [外部写入接入边界](EXTERNAL_WRITE_BOUNDARY.md)                              | 说明外部写入应复用标准模块动作、权限、租户、校验、生命周期和运行审计；幂等、回执、重试和报文治理按触发条件后续建设。 |
| [后台任务与异步批次边界](BACKGROUND_JOB_AND_BATCH_BOUNDARY.md)              | 说明同步批量动作、异步导入、补偿任务、进度查询、重试和批次模型的建设触发条件。                                       |
| [动作结果与数据变更契约](ACTION_RESULT_AND_DATA_CHANGE.md)                  | 说明业务动作成功回执、可读消息、数据变化事实、HTTP 包装和异步广播之间的统一契约。                                    |
| [平台实时通信设计](REALTIME_COMMUNICATION.md)                               | 说明 WebSocket/STOMP 实时通信底座、前后端封装边界、数据变化广播、双向通信预留和后续可靠性演进。                      |
| [实时业务提醒后端接入](BUSINESS_NOTIFICATION_BACKEND_INTEGRATION.md)        | 说明业务服务发布在线提醒、按 IAM 范围接收、导航/命令动作和可靠性边界。                                               |
| [MuYunFileServer 与文件引用生命周期](FILE_REFERENCE_LIFECYCLE.md)           | 说明关联文件服务、记录附件、字段文件引用、临时上传、转正和显式物理删除的职责边界。                                   |
| [智能助手能力融合专项治理](AI_ASSISTANT_GOVERNANCE.md)                      | 说明面向终端用户的助手目标场景、前后端分层、前端 Surface 与能力贡献、动态配置路线和阶段验收口径。                    |
| [AI 应用建设与业务协作专项规划](AI_COLLABORATION_ROADMAP.md)               | 说明共同基础、辅助建设应用、受控业务操作、任务协作和后台外部协作五阶段的目标、思路、职责与验收边界。               |

## 平台运行模式

平台运行模式通过 `muyun.runtime.mode` 配置，当前只区分 `development` 和 `production`，未配置时默认 `production`。

运行模式只提供平台治理默认值，不替代具体能力的显式边界。当前 schema migration 默认策略是：开发态默认 execute，产品态默认 strict execute；调用方显式传入 `MigrationOptions` 时以显式选项为准。元数据结构保存、动态模块 refresh 和平台级 schema ensure 应沿用该默认策略；preview refresh 始终 dry-run。平台托管数据修改仍以 `PlatformManagedMutationContext`、租户系统态和对应 Ability 契约为准，不因为运行模式切换而放开。

## 写作原则

1. 以业务能力梳理为主，少写实现细节。
2. 接口文档按 URL 说明功能点，不追求完整 OpenAPI。
3. 能由代码和测试证明的细节，不在文档中重复展开。
4. 明确当前边界和不承诺项，避免读者误以为阶段性能力已经变成完整产品能力。
