...
| TD-050 | 动态元数据能力的编辑会话与原子发布尚未完成 | `Metadata.capabilityDeclarations` 已成为树、排序、启停三项动态字段能力的唯一声明来源；能力目录统一描述 MAIN/CHILD 适用范围、依赖、平台字段贡献与默认值语义，`null` 仅作为存量字段推导的过渡边界。模块关系只提供入口与角色校验，不保存能力状态。数据权限、审批与引用仍沿用既有受控配置/运行态契约 | 进入真实编辑场景时，补齐编辑会话、变更预览确认、跨模块影响提示、原子发布/Schema ensure/运行态刷新与禁用保留策略；首期仅允许主元数据开启树、排序、启停，数据权限、审批与引用继续按现有受控边界治理 |
...
| TD-040 | 管理工作台真实业务页面的浏览器覆盖仍是增量工作 | 已通过 Vitest Browser Mode 覆盖共享工作区的桌面满高、内部滚动和 980px `listSurface` 回退，避免 jsdom 无法验证 DOM 几何的问题；但它使用受控组件 fixture，不承载登录、Demo 数据装配和各业务页面的端到端交互 | 后续涉及工作区路由恢复、真实动态列表/表格或复杂多栏交互时，以对应稳定业务样板追加浏览器场景；保持测试只锁定布局与滚动契约，不把全部管理交互迁入浏览器测试 |
