...
| TD-040 | 管理工作台仍缺真实整页布局验收矩阵 | 已有 Vitest Chromium 浏览器入口，覆盖树/平铺列表拖拽、动效、拖拽滚动、长标签保护标记及工作区鼠标/键盘互斥；另有需显式启用真实服务的树移动测试。上述组件级验证不能替代完整 Demo 页面的高度传递、滚动所有者和窄屏布局验收 | 下一次调整工作台布局时，以真实 Demo 页面补齐树、列表/表格、详情三类区域的桌面滚动和 980px 窄屏回退，验证 clientHeight、scrollHeight 与实际滚动容器；复用已有浏览器测试设施 |
