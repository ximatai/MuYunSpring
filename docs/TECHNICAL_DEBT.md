...
| 编号 | 议题 | 当前口径 | 触发条件 |
| --- | --- | --- | --- |
| DD-004 | 平台错误反馈的场景化定位与展示尚未完整接入 | 后端统一 envelope、traceId、基础错误码、前端 `AppError`、全局展示槽位判定和登录特殊链路已落地；管理工作台已有 `presentPlatformError` 与真实消息提示，动态页面宿主也已接入。当前通用反馈除静默/登录跳转外主要归一为消息提示，并未完整区分 modal/page-error/字段定位。动态表单已有必填、只读的字段/子表 target 契约，但其余动态 local edit、bootstrap、查询、导入和配置解析仍需按实际场景统一异常 code、HTTP 状态和局部接管语义 | 在真实字段/子表校验定位、查重确认、导入行级错误或工作流动作反馈场景中，复用已有反馈入口，补对应 handler、局部消费和避免重复全局提示的契约；不再把基础消息 UI 视为未建设能力 |
...
| TD-040 | 管理工作台仍缺真实整页布局验收矩阵 | 已有 Vitest Chromium 浏览器入口，覆盖树/平铺列表拖拽、动效、拖拽滚动、长标签保护标记及工作区鼠标/键盘互斥；另有需显式启用真实服务的树移动测试。页面编排工作台已补齐真实组件组合的 1440px/980px 长树、长表单滚动及首次双击命中验证；其他完整 Demo 页面的高度传递、滚动所有者和窄屏布局仍需继续验收 | 下一次调整工作台布局时，以真实 Demo 页面补齐树、列表/表格、详情三类区域的桌面滚动和 980px 窄屏回退，验证 clientHeight、scrollHeight 与实际滚动容器；复用已有浏览器测试设施 |
...
| DD-003 | 配置删除的全量引用治理范围 | 当前元数据、元数据字段、模块字段和模块元数据关系已采用统一贡献式引用检查：被已登记的配置引用时返回 `RESOURCE_IN_USE`（409），不产生悬空运行态。页面定义的主实体绑定，以及草稿/已发布修订的列表字段、表单字段、分组字段和直接子实体关联已通过页面域贡献接入统一删除守卫；历史归档不阻塞字段演进。编排器保留失效来源、阻止带失效来源发布，并支持显式移除后修正。表达式、多跳字段路径和配置包差异等间接引用仍未完整覆盖 | 公式、多跳引用或配置包能力进入真实场景时，继续按同一贡献契约补齐语义解析；历史修订重新启用须重新校验当前来源，不能绕过发布校验 |
