...
当前前端平台交付面已具备管理型 App 的基础承载能力。静态模块与动态模块共享标准页面宿主；动态管理页已有已发布页面编排驱动的主子记录读写闭环，具体交付范围以 configuration/page 专题与契约测试为准：

| 方向                 | 当前状态                                                                                                                                                                            | 后续触发                                                                                                |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| Workbench            | 登录、token 保存、session/menu 启动、菜单页签、URL 恢复、mock client、后端模式构建已有测试或构建覆盖。                                                                              | 进入全局通知、租户切换、用户偏好或生产级登录安全时，再补真实 UI 出口和会话治理。                        |
| 页面交付后端契约     | `page/delivery` 后端接口已形成阶段契约，覆盖 bootstrap、查询、保存、附件、查重、引用、导航和偏好；接口细节以 [页面交付 Web API](../platform/topics/page/delivery/WEB_API.md) 为准。 | 已有消费链路通过 `web-contracts` 和 `web-core` 接入；后续按场景补齐尚未交付的页面能力。           |
...
| dynamic-page-runtime | 标准模块宿主承载列表、树、详情与编辑会话；动态管理页消费已发布编排，支持主子记录查询、保存与版本冲突处理。                                                              | 继续验证来源无关的页面契约与受控扩展，按场景补齐局部错误和其他交付能力。 |
| platform-components  | 已沉淀查询列表、记录表单与详情、引用选择和工作区等平台组件，标准宿主与平台管理页共同消费。                                                                                                                                | 引用选择、附件面板、导入导出、审批动作区等至少出现稳定跨项目复用语义后再进入。                          |
| 业务示例             | `examples/business-web` 验证业务项目可通过本地 alias 消费平台包，且不直接依赖 Ant Design Vue。                                                                                      | 平台包正式拆分或业务团队开始独立仓库接入时，补私有 npm 发布和迁移说明。                                 |

动态页面编排、字段编辑和保存校验已有阶段交付；其余页面能力按真实业务场景补齐配置、运行态与交互契约，不把底层接口存在等同于完整交付。

`RecordQueryListSurface` 是列表页面的纯展示骨架，统一承载标题、同一行的操作与快速查询、表格和页脚。`RecordQueryListPanel` 保留查询、权限、动作和分页生命周期；页面配置草稿预览只传入已编译 descriptor、示例记录和编辑插槽，不构造模块运行上下文，也不执行业务请求或动作。
...
当前重点是收敛标准模块宿主、页面编排和配置发布的一致性，并验证管理型 App 的接入成本。已交付的主子记录闭环继续复用统一运行器，剩余页面语义按真实业务场景逐步闭合。
