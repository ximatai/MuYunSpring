...
`RecordQueryListSurface` 统一承载列表操作、查询条件、表格和分页。列表标题只表达当前页面内额外的局部语境：工作台页头已经完整表达页面身份、且页面只有一个主列表时，应隐藏重复的列表标题；主从布局、关联列表或同页多列表仍可使用列表标题区分上下文。无标题列表不保留空标题占位，业务操作区靠左、查询区靠右；宿主宽度不足时查询区优先换行，不能通过压缩标题或操作按钮换取单行布局。布局由公共组件负责。
