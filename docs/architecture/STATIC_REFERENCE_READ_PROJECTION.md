...
标准列表与详情通过引用 descriptor 的 `titleField` 消费已授权的标题或结构化摘要；不为只读展示调用候选接口。
结构化多值摘要按持久化 ID 对齐并保留顺序，缺少摘要时仍显示该 ID，不丢弃其他引用，也不据此推断目标已删除或无权读取。
来源值为空时旧摘要不参与展示。编辑提交过滤 descriptor 声明的读投影输出，显式字段回填与持久化引用 ID 保持各自写入语义。
