...
字段引用配置独立表达“源字段 -> 目标实体”的引用关系，以及标题输出和投影输出；同样支持默认配置和关系级覆盖。动态目标继续以 `targetMetadataId` 定位，并受目标元数据绑定约束；静态目标不伪造元数据，而是以完整静态模块 alias（如 `education.student`）解析其已注册的 `ReferenceTarget`。静态目标配置必须是关系级配置，且仅允许指向可解析的静态 `ReferenceAbility`；其固定字段投影、候选、标题、带出、写入完整性和删除约束都复用该统一能力。子表归属外键仍只能指向所属动态父实体，不能借静态目标配置绕过聚合约束。管理页面可以在只读展示位使用已声明 `ONE` 引用的点路径，例如 `supplierId.organizationId.title`；路径在页面编译期验证，按共享引用读投影批量执行，不形成元数据派生字段，也不参加保存、筛选或排序。除此之外，跨动态模块展示投影尚未开放，不能借本契约扩展为任意跨模块 DSL。运行态刷新时由字段引用配置编译成运行态 `EntityReferenceDefinition`。运行态 descriptor 输出模块、实体、字段、关系、引用、动作、基础视图和关联视图；动作统一由模块动作目录表达，可覆盖标准动作的启停、标题、级别和权限挂点，也可声明自定义动作。元数据描述数据结构，不再拥有独立对外动作模型；子元数据如需暴露对外动作，应显式追加到模块动作目录并指定目标实体。模块动作的 `actionCode` 在模块内唯一，对外 URL、权限授权和执行定位都以这个口径为准，不再通过实体维度区分同名动作。基础视图只表达列表/表单字段顺序、显示状态、标题、兼容控件类型和 UI 字段类型，不承载复杂布局。
