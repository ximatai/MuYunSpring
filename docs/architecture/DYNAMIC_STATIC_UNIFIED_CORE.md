...
静态标准模块的读投影消费已编译执行计划，不能以模块专属兼容标记在请求期重新解释 DSL。动态标准列表、导航来源查询和保存字段约束同样消费已安装计划；独立测试应构造最小执行计划，不通过生产 fallback 读取配置快照。静态只读交付复用相同的查询、页面范围、读投影和 wire 编码契约，仅暴露查询与详情端点。

标准运行时缺少执行计划统一返回 `CONFIG_MISSING`。动态请求缺少计划要求的 UI 配置标识属于 `VALIDATION_FAILED`；提供的标识与当前已发布计划不一致属于 `CONFLICT_VERSION`，不能当作平台配置缺失。非法查询字段或模板仍按请求校验失败处理。

动态关联查询与诊断的实体级过滤不依赖目标模块发布独立管理页面；只有使用目标页面的查询表单、模板、快捷搜索或显式 UI 配置时，才要求目标页面执行计划。关联读取继续由实体运行态校验目标字段、关联范围和访问权限。
...
一层直接子资源通过受管 detail relation 接入同一标准宿主。Web DSL 分别声明可执行 query、可选 mutation、父记录绑定和来源无关适用条件，并在启动期编译出固定 association 协议、动作码、查询/列表/编辑字段事实；运行期只消费冻结计划。后端网关先按父场景的数据权限读取持久化父记录（读为 `VIEW`，写为 `UPDATE`），再绑定父键并校验 child 归属、子资源动作权限、数据权限和写字段白名单。预解析的数据范围只能作为与 Service 实例、操作和记录精确绑定的临时执行上下文传递，网关最终仍调用 Service 的正常多态入口，不得绕过软删、领域 override 或能力组合生命周期。前端标准 surface 独占分页查询、字段 codec、validity、busy/关闭门禁、错误、局部刷新和父切换的陈旧响应隔离，业务不拼 URL、不预载专用选项，也不把模块特判放进 Host。独立可写 handler 只接受静态 Service adapter；动态模块的一层直接子表采用下述聚合保存契约，不通过独立 relation mutation 写入。多层嵌套、树、批量、拖拽、任意组件和组合范围导航不属于该契约。

受管 relation 的编辑与保存语义必须显式声明。`INDEPENDENT` 保留子资源独立命令；聚合子表只在父记录编辑期间提供行内草稿，单元格输入、新增和移除均不发写请求，主表取消会丢弃全部子表草稿。父模型通过 `@Children` 暴露集合，子模型通过 `@ChildOf` 声明归属，标准父模块 `view/insert/update` 负责装配和提交完整聚合，`ChildrenAbility` 在同一事务中委派 `ChildAbility` 执行子记录生命周期；前端不得调用独立 relation 或 aggregate 特殊保存接口。列表查询默认不装配子集合，避免把详情负担扩散到列表。聚合草稿支持静态子资源和声明了物理父键的动态一层直接子表，动态页面由已发布执行计划校验字段、父子归属和权限；静态聚合可声明受限的行间回写公式：`others({positions.primaryPosition}) = false WHEN {positions.primaryPosition}`。它以唯一的用户变更行作为 source，只能回写同一直接子表的其他行：服务端在保存时以同一 FormulaEngine 再次调和，描述符则下发已编译的 `FORM_COMPUTE` 程序，由浏览器在编辑时立即执行。浏览器不重新解析表达式，也不承载业务特判。动态独立 relation mutation、多层嵌套和批量粘贴仍不在稳定契约内。
...
| 类型 | 含义 | 当前能力 |
| --- | --- | --- |
| 标准运行保障 | 标准入口统一执行，不由业务复制流程 | CRUD、版本控制、生命周期、宿主事务、已声明规则的执行 |
| 业务选择 | 按生命周期、结构和访问需要显式接入 | 软删除、回收站、启停、树、排序、数据范围、引用目标 |
| 模型事实与约束 | 声明一次，平台编译和执行 | 唯一约束、排序分区、引用完整性、关联读字段、父子关系 |
| 运行优化 | 可选策略，不改变业务生命周期 | 缓存及其自动依赖维护 |

静态标准基类不会自动启用软删除或缓存；普通可维护资料推荐显式组合软删除，硬删除绑定表等保留自身选择。动态 `EntityCapability.BASELINE` 是动态定义的归一化分类，当前包含 CRUD、生命周期、软删除和缓存，不代表静态 Service 自动实现这些接口。两种接入可有不同默认组合，启用后的行为共享同一能力实现。
...
10. 静态引用依赖从模型 `@ReferenceTo` 自动采集；`ReferencerAbility` 供动态运行态或自定义引用行为扩展。

声明唯一约束的标准 DAO 写入在保存点内执行；数据库拒绝写入时先回滚该语句，再解析冲突及回收站定位信息。保存点不独立提交资源，也不包含生命周期回调，业务失败仍回滚整次标准写入。
...
缓存能力先作为显式能力挂载：服务实现 `CacheAbility` 后，标准 `select(id)` 可复用缓存，写链路在 `afterChanged` 之后由 CRUD 内部统一失效。静态服务默认缓存命名空间包含服务类、模块别名和 DAO 实例；动态运行态缓存命名空间在同一 `DynamicRecordRuntime` 内按模块和实体稳定，在不同运行态之间隔离。缓存对象必须通过 `copyForCache` 进出，避免调用方修改返回对象污染缓存内容。跨模型引用缓存失效已有本地进程内闭环：平台从静态模型声明或动态引用契约采集直接引用依赖，目标记录变更时清理引用方缓存；普通静态业务无需额外实现 `ReferencerAbility`。跨节点治理后续再升级。
...
导航来源不另设能力枚举或重复声明。静态来源的列表/树读取形态由已注册的 `REFERENCE` 端点证明，动态来源从已发布实体的引用与树能力推导；静态页面在端点注册完成后校验来源，动态页面在发布时校验。读取权限与写动作权限分别执行，导航中的管理交互仍服从来源模块的标准动作。

导航树的排序复用标准 `SORT` Operation 和 `/sort/{id}` 端点，不增加导航专用写能力。导航上下文以 `TreeSortWebRequest.scope` 传入，只承载外部范围值和宿主导航定位；它不是列表查询条件。支持范围的树 Web 投影归一化该上下文后，继续执行标准租户、动作权限、记录范围、树移动和变更事实链路。动态树排序从已编译的页面上下文绑定解析范围，并通过同一 TreeAbility 的范围移动入口执行；移动节点、目标父节点、相邻节点和实际重排集合均受范围约束。必填上游范围缺失必须拒绝，不能静默忽略。未实现范围投影的入口必须拒绝携带 scope 的排序请求。

树排序分区由实际被操作的实体定义提供。树子资源在编译时必须解析到实体定义，并将分区写入资源 descriptor；普通树和导航树使用自身模块主实体的分区。前端树组件只消费显式传入的分区，空列表表示合法无业务分区，缺失表示契约尚未解析，不能回退到其他宿主实体或开放拖拽。

静态动作的可执行集合及身份以静态声明为准，持久化配置仅覆盖同动作的启停及治理选项，不凭额外数据库记录创建静态动作。运行态描述、HTTP 动作解析与 OpenAPI 应遵守同一声明和覆盖语义；配置停用动作后，既有 HTTP 映射可以保留，但请求必须拒绝，描述和文档不再暴露该动作。
...
动态元数据的破坏性 schema 治理（删列、删字段、停用会移除物理列的能力）以全表物理事实为准，不能受操作者的数据范围、租户或软删除过滤影响；当前仅在 PostgreSQL 上以事务内表锁实现该事实检查与 DDL 的互斥，其他数据库必须显式拒绝，不得降级为无锁执行。
...
能力字段属于平台契约，不属于业务配置自由项。静态模型通过 Java 字段和注解声明能力字段；动态模型一旦开启字段声明能力，必须使用同一组标准字段名、列名和类型。例如树能力统一使用 `parentId` / `parent_id`，并自动包含排序能力的 `sortOrder` / `sort_order`；启停能力统一使用 `enabled` / `enabled`，但不属于基线字段，也不参与平台默认过滤。`tenantId` / `tenant_id` 属于所有平台实体的基础字段，不由业务元数据重复声明。动态模型会将所有可参与 UI 编排的基础字段和能力字段固化为 `STANDARD + systemManaged` 的 `metadata_field` 条目，供字段树和 UI 配置以稳定字段 ID 绑定；这些条目是运行态标准 schema 的投影，不是另一套物理列定义。后续工作流等需要标准字段的能力也按同一原则处理；动态定义默认归一软删除、生命周期和缓存；静态 Service 的软删除和缓存仍按业务显式选择。

租户过滤属于默认 Ability 作用域，不属于业务 DAO 的手写条件。运行时存在当前租户时，插入会补齐 `tenantId`，默认查询、分页、计数、按 ID 读取、更新和删除都应在同一租户作用域内执行；无租户上下文时按系统态处理。

租户业务的页面执行作用域由平台统一提供，不依赖业务 UI 是否声明租户导航。静态服务与动态元数据归一到模块作用域契约，普通业务 HTTP 入口要求明确的有效租户；系统资源及跨范围治理须保持显式作用域边界。标准页面宿主在业务导航左侧提供租户列表，静态页面、动态页面和业务预览共用候选、选择和请求作用域。业务仅声明机构、分类等租户内导航；切换租户重置当前页面的业务会话，不改变登录身份或其他页签。
