...
静态标准模块的读投影消费已编译执行计划，不能以模块专属兼容标记在请求期重新解释 DSL。动态标准列表、导航来源查询和保存字段约束同样消费已安装计划；独立测试应构造最小执行计划，不通过生产 fallback 读取配置快照。静态读 transport 的其余适配差异保留在 `TD-043`，不为消除表面重复而抹平真实交付边界。
