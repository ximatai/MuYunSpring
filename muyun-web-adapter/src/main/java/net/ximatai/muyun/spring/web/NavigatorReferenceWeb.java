        ...
        return webScope(() -> {
            ...
            if (service() instanceof DataScopeAbility<?> dataScopeAbility) {
                ...
                PageResult<T> scoped = (PageResult<T>) DataScopeAbility.cast(dataScopeAbility).pageQueryForAction(
                        PlatformAction.REFERENCE, queryCriteria(request), pageRequest, querySorts(request));
                result = scoped;
            } else {
                result = service().pageQuery(queryCriteria(request), pageRequest, querySorts(request));
            }
