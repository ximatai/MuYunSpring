...
public record LogText(String value, boolean truncated, boolean redacted) {
    ...
    private static final Pattern NAMED_SECRET = Pattern.compile(
            "(?i)\\b(password|passwd|pwd|token|authorization|secret|credential|cookie)\\b\\s*([=:])\\s*([^\\s,;}&]+)");
    ...
    public static LogText of(String rawValue) {
        ...
        String redactedValue = NAMED_SECRET.matcher(rawValue).replaceAll("$1$2" + REDACTED);
