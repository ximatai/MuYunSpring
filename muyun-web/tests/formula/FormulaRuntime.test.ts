...
describe('FormulaRuntime FORM_COMPUTE', () => {
  it('evaluates a deterministic assignment locally without mutating the draft', () => {
    ...
    const result = runtime.evaluateFormCompute(
      program(
        assign(
          field('amount'),
          binary(
  ...
  it('aligns empty values, loose equality, IN and zero division with FormulaEngine semantics', () => {
    ...
    expect(
      runtime.evaluateFormCompute(
        program(assign(field('amount'), binary('/', value(9), field('divisor')))),
        draft,
      ),
  ...
  it('fails closed for malformed, unsupported and unchanged programs', () => {
    expect(runtime.evaluateFormCompute(program(assign(field('amount'), value(1))), { amount: 1 })).toEqual({
      patch: {},
      changedFields: [],
    });
    expect(
      runtime.evaluateFormCompute(
        { ...program(assign(field('amount'), value(1))), schemaVersion: 2 },
        { amount: 0 },
      ),
