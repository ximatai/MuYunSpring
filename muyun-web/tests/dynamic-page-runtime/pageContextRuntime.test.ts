...
describe('resolvePageContextTargetValues', () => {
  const bindings = [
    ...
    { source: 'NAVIGATOR', sourceKey: 'tenant', target: 'NAVIGATOR_QUERY', targetKey: 'tenantId', targetNavigatorLevelKey: 'category' },
  ...
  it('combines values from every declared source for a target', () => {
    expect(resolvePageContextTargetValues([...bindings], 'LIST_QUERY', {
  ...
  it('keeps navigator query flows scoped to the declared downstream level', () => {
    expect(resolvePageContextTargetValues([...bindings], 'NAVIGATOR_QUERY', {
      NAVIGATOR: { tenant: 'xcmg' },
    }, 'category')).toEqual({ tenantId: 'xcmg' });
    expect(resolvePageContextTargetValues([...bindings], 'NAVIGATOR_QUERY', {
      NAVIGATOR: { tenant: 'xcmg' },
    }, 'other')).toBeUndefined();
  });
